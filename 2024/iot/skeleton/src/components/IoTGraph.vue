...
const x = d3
  .scaleTime()
  .domain([
    dayjs.unix(Math.min(...props.data.map((v) => v.getTimestamp()))).toDate(),
    dayjs.unix(Math.max(...props.data.map((v) => v.getTimestamp()))).toDate(),
  ])
  .range([0 + margin.left, width]);

const y = d3
  .scaleLinear()
  .domain([Math.max(...props.data.map((v) => v.getValue())) + margin.top, 0])
